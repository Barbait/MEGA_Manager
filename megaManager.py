...
class MegaManager(object):
                            ...
                            if file_subPath is not '':
                                ...
                                if path.isfile(local_filePath):
                                    if (local_filePath not in self.__compressedVideoFiles) \
                                        ...
                                        logger.debug(
                                            ' Error, video file previously processed. Moving on. "%s"!' % local_filePath)
    ...
    def _get_profile_details(self, profile):
        ...
        profile.account = self._update_account_remote_details(account=profile.account)
    ...
    def _update_account_remote_details(self, account):
        ...
        account.remote_totalSpace = self.__megaTools.get_account_total_space(username=username, password=password)

        account.remote_freeSpace = self.__megaTools.get_account_free_space(username=username, password=password)

        account.remote_usedSpace = self.__megaTools.get_account_used_space(username=username, password=password)
